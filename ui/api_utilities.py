...
async def create_session_conversation(
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            f"{API_BASE_URL}/session_conversations/", json=payload
        )
...
async def get_session_conversation(conversation_id: int) -> Dict[str, Any]:
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{API_BASE_URL}/session_conversations/{conversation_id}"
        )
        response.raise_for_status()
        return response.json()
...
async def get_session_conversations(
    skip: int = 0, limit: int = 100, user_id: Optional[str] = None
):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{API_BASE_URL}/session_conversations/", params=params
        )
        response.raise_for_status()
        return response.json()
...
async def update_session_conversation(
    conversation_id: int, update_fields: Dict[str, Any]
) -> Dict[str, Any]:
    async with httpx.AsyncClient() as client:
...
async def load_user_conversations(user_id: str):
    all_sessions = await get_session_conversations(
        user_id=user_id
    )  # Filter on user_id backend side
...
async def get_next_session_id(user_id: str) -> int:
    ...
    session_ids = [s.get("session_id")
                   for s in sessions if s.get("session_id") is not None]
    if session_ids:
...
async def verify_totp_code(username, code):
    ...
    user_with_totp = next(
        (user for user in users if user.get("totp_secret") is not None), None
    )
