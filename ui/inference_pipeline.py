...
def run_inference(user_prompt):
    ...
    tfidf_features = len(
        checkpoint["vectorizers"]["tfidf"].get_feature_names_out())
    count_features = len(
        checkpoint["vectorizers"]["count"].get_feature_names_out())
    ...
    features = emotion_model.enhanced_feature_preparation(
        demo_df, fit_vectorizer=False, augment=False
    )
    ...
    return {"emotion": emotion, "strategy": strategy, "answer": answer}
