...
>>>>>>> 2d0754f84886a797b0946859fc543d4717de4de9
...
if latest_assistant_content is not None:
    ...
    with col1:
        if st.button("Shorten"):
            try:
                ...
                clean_answer = re.sub(r'\*\*.*?\*\*', '', latest_assistant_content)
                prompt = (
                    f"Topic: {topic}\n\n"
    ...
    with col2:
        ...
        if st.button("🔊 Listen", key=audio_key):
            ...
            import re
            match = re.search(r'\*\*Answer:\*\*\s*(.*?)(\n\n|$)', latest_assistant_content, re.DOTALL)
