...
# Hide video and header for clean UI
st.markdown(
    """
    <style>
...
def reset_all_states():
    # Reset all Streamlit session state keys
...
if not st.session_state.authenticated:
    ...
    st.markdown(
        '<div class="login-title">🔐 Login or Register</div>',
        unsafe_allow_html=True)
    st.markdown(
        '<div class="login-subtext">Welcome to <b>AL_Git Chatbot</b>. Please log in or create an account to continue.</div>',
        unsafe_allow_html=True,
    )
    ...
    if auth_mode == "Register":
        if st.button("📝 Register", type="primary"):
            ...
            if success:
                ...
                # Prepare payload data for new session conversation with empty
                # messages
                ...
                **How to use Google Authenticator:**
                ...
                try:
                    initial_session_id = asyncio.run(
                        get_next_session_id(user_id))
                ...
                try:
                    ...
                    st.success(
                        f"User session entry created successfully with session_id {initial_session_id}."
                    )
                    # Update session id mapping in state if needed
                    st.session_state.sessionid_to_id[str(initial_session_id)] = (
                        created_session.get("id"))
                    # Also add to conversations state
                    st.session_state.conversations[str(
                        initial_session_id)] = []
                except Exception as e:
        ...
        if st.button("🔓 Login", type="primary"):
            ...
            if not users:
                ...
                for user_rec in users:
                    ...
                    if (
                        user_rec.get("password") is not None
                        and user_rec.get("password") == password
                    ):
        ...
        if st.session_state.pending_2fa:
            ...
            if st.button("✅ Verify 2FA", type="primary"):
                if asyncio.run(
                    verify_totp_code(st.session_state.pending_username, code)
                ):
                    # if 1 == 1:
                    ...
                    st.session_state.conversations, st.session_state.sessionid_to_id = (
                        asyncio.run(load_user_conversations(st.session_state.user_id))
                        )
                    st.rerun()  # better practice than st.rerun()
                    if not st.session_state.conversations:
                        # If user has no conversation, create a blank one with
                        # session_id=1 or generated
                        ...
                        # await create_session_conversation(**empty_session) -
                        # async handled appropriately
                        st.session_state.conversations = {
                            str(initial_session_id): []}
                    st.session_state.current_conv = list(
...
# Initialize session state for conversations store and current selected
# conversation
if "conversations" not in st.session_state:
    st.session_state.conversations = {
...
selected_conv = st.sidebar.radio(
    ...
    conversation_names,
    index=(
        conversation_names.index(st.session_state.current_conv)
        if st.session_state.current_conv in conversation_names
        else 0
    ),
)
...
if st.session_state.page == "Unity Game":
    ...
    server_command = ["python", "-m", "http.server", "8502"]
    server_cwd = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "Unity_game")
    )
    try:
        subprocess.Popen(
            server_command,
...
messages = st.session_state.conversations.get(
    ...
    if messages:
        ...
        for msg in reversed(messages):
            if msg.get("role") == "user" or msg.get("owner") == "user":
                last_user_message_content = msg.get(
                    "content") or msg.get("message")
...
for message in messages:
    with st.chat_message(
        message.get("role", message.get("owner", "user"))
    ):  # fallback role
...
if prompt:
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Thinking..."):
            ...
            same_topic_count = count_consecutive_same_topic(
                messages, st.session_state.topic_model, threshold=0.4
            )
            ...
            if is_new_topic(
                ...
                last_user, last_assistant = asyncio.run(
                    get_last_user_and_assistant(messages)
                )
    ...
    if db_id is None:
        ...
        asyncio.run(
            update_session_conversation(
                conversation_id=db_id, update_fields=update_fields
            )
        )
...
if messages and messages[-1]["role"] == "assistant":
    ...
    with col1:
        if st.button("Export Conversation as PDF"):
            ...
            with open("conversation_export.pdf", "rb") as f:
                st.download_button(
                    "Download PDF", f, file_name="conversation_export.pdf"
                )
    ...
    with col2:
        ...
        if st.button("🔊 Listen", key=audio_key):
            ...
            match = re.search(
                r"\*\*Answer:\*\*\s*(.*)", latest_assistant_content, re.DOTALL
            )
            answer_text = match.group(1).strip(
            ) if match else latest_assistant_content
...
if messages and messages[-1]["role"] == "assistant":
    ...
    if not st.session_state.is_ai_thinking:

        if st.button("Yes", key="feedback_yes"):
            print("DEBUG: User chose YES")
            st.session_state.is_feedback_postive = True
            st.session_state.show_secondary_feedback_options = (
                False  # Ensure this is hidden
            )
            st.rerun()
        ...
        if st.button("No", key="feedback_no"):
            ...
            st.session_state.show_secondary_feedback_options = (
                True  # Show secondary options
            )
            # Here you would typically send negative feedback to your
            # backend/log

    if st.session_state.is_feedback_postive:
    ...
    if st.session_state.show_secondary_feedback_options:
        ...
        if st.button("Shorten", key="secondary_shorten"):
            try:
                ...
                db_id = st.session_state.sessionid_to_id.get(
                    st.session_state.current_conv
                )
                if db_id is None:
                    st.error("Cannot find conversation id in session mapping!")
        ...
        if st.button("Change Source", key="secondary_source"):
            ...
            if original_prompt_for_llm:
                ...
                st.session_state.is_ai_thinking = True
                with st.spinner(
                    "Generating answer from a different perspective/source..."
                ):
                    result = run_inference(new_prompt)
                response = result["answer"]
                messages.append({"role": "assistant", "content": response})
                ...
                st.warning(
                    "Cannot change source: original prompt not available.")
